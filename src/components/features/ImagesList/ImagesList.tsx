...
const ImagesList = ({ searchQuery }: ImagesListProps) => {
  ...
  const { modalIndex, openModal, closeModal, handlePrev, handleNext } = useModalGallery(photos);

  const [fetchPhotos, isLoading, error] = useFetching( async(query: string | undefined , page: number, perPage: number ) => {
  const images = query
