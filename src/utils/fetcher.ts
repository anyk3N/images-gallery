...
export async function fetchCollections(): Promise<UnsplashPhoto[]> {
  const response = await fetch(`${apiUrl}/collections?per_page=12&client_id=${client_id}`);
...
export async function fetchPhotosByCategory(category: string, page = 1, perPage = 12, orderBy = "relevant"): Promise<UnsplashPhoto[]> {
  const response = await fetch(
    `https://api.unsplash.com/search/photos?query=${category}&page=${page}&per_page=${perPage}&order_by=${orderBy}&client_id=${client_id}`
  );
...
export async function fetchPhotos(page = 1, per_page = 12,orderBy = "relevant"): Promise<UnsplashPhoto[]> {
  const response = await fetch(`${apiUrl}/photos?page=${page}&per_page=${per_page}&order_by=${orderBy}&client_id=${client_id}`);
  const data = await response.json();
  return data;
}